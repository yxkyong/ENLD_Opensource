...
################################################ load data
    ...
    # k = 1
        ...
        sampled_x, sampled_y = enld_function.contrastive_sampling_sub_onlypositive(sub_KD_tree_list, sub_high_quality_dic, incremental_am_feature,am_y,P_observed_true, incremental_am_feature.shape[0]*size, noisy_y_set)
        ...
        for i in range(iteration):
            ...
            am_pre = pre_label[am_index]
            incremental_am_feature = incremental_feature[am_index] #enld_function.feature_extraction(res_model, am_x, layer_name = layer_name)[1]
            sampled_x, sampled_y = enld_function.contrastive_sampling_sub_onlypositive(sub_KD_tree_list, sub_high_quality_dic, incremental_am_feature,am_y,P_observed_true,am_x.shape[0]*size, noisy_y_set)
            sampled_x = flatten(sampled_x)
            sampled_y = flatten(sampled_y)
            sampled_y_one_hot = get_all_one_hot(sampled_y, class_num=n_classes)
            print(sampled_x.shape,sampled_y_one_hot.shape)
