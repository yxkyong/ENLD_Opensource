...
def random_policy(sub_x, sub_y, size):
    # pass
    list_whole = [i for i in range(len(sub_y))]#np.array([i for i in range(len(sub_y))])
    # print(list_whole,size)
    index = random.sample(list_whole, size)
...
################################################ load data
    ...
    # k = 1
        ...
        vote = args.vote
        filter_para = 0.6
        name = './tmp/tmp_mix_size1_random.h5'
        ...
        sampled_x, sampled_y = random_policy(sub_x, sub_y, incremental_am_feature.shape[0]*size)
