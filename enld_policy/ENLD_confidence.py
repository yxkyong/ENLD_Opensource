...
def get_high_quality_data_only_confidence_auto_return(res_model, X_test, Y_test, layer_name, confidence=0, auto=False):
    ###################### return all
    pred_probs, feature = enld_function.feature_extraction(res_model, X_test, layer_name = layer_name)
    pred_probs_whole = pred_probs
    observe_labels = np.argmax(np.array(Y_test), axis=1)
    high_quality_index = enld_function.get_high_quality_confidence_auto(observe_labels, pred_probs, auto=auto)
    high_quality_x = X_test[high_quality_index]
    high_quality_y = observe_labels[high_quality_index]
    ...
    return high_quality_x,high_quality_y,high_quality_index,high_quality_y_onehost, return_feature, pred_probs_whole   

def return_topk_index(Lst, k):
    sort_Lst = sorted(Lst)
    sort_Lst.reverse()
    index_k = []
    for i in range(k):
        index_i = Lst.index(sort_Lst[i])    
        index_k.append(index_i)
    return index_k

def confidence_policy(sub_x, sub_y, size, pred_probs_whole):
    # observe_labels = np.argmax(np.array(sub_y), axis=1)
    max_confident_list = []
    for i in range(pred_probs_whole.shape[0]):
        max_confident_list.append(np.max((pred_probs_whole[i])))
    
    index = return_topk_index(max_confident_list,size)
    sample_x = sub_x[index]
    sample_y = sub_y[index]   
    return sample_x, sample_y

def confidence_policy_pseudo(sub_x, sub_y, size, pred_probs_whole):
    predicted_label = np.argmax(np.array(pred_probs_whole), axis=1)
    max_confident_list = []
    for i in range(pred_probs_whole.shape[0]):
        max_confident_list.append(np.max((pred_probs_whole[i])))
    ...
    index = return_topk_index(max_confident_list,size)
    sample_x = sub_x[index]
    sample_y = predicted_label[index]   
    return sample_x, sample_y

def return_lower_index(Lst, k):
    sort_Lst = sorted(Lst)
    # sort_Lst.reverse()
    index_k = []
    for i in range(k):
...
def confidence_policy_active(sub_x, sub_y, size, pred_probs_whole):
    # observe_labels = np.argmax(np.array(sub_y), axis=1)
    max_confident_list = []
    for i in range(pred_probs_whole.shape[0]):
        max_confident_list.append(np.max((pred_probs_whole[i])))
    
    index = return_lower_index(max_confident_list,size)
    sample_x = sub_x[index]
    sample_y = sub_y[index]   
    return sample_x, sample_y

from scipy.stats import entropy
def entropy_policy(sub_x, sub_y, size, pred_probs_whole):
    # observe_labels = np.argmax(np.array(Y_test), axis=1)
    max_confident_list = []
    for i in range(pred_probs_whole.shape[0]):
        max_confident_list.append(entropy(pred_probs_whole[i]))
...
################################################ load data
    ...
    # k = 1
        ...
        vote = args.vote
        filter_para = 0.6
        name = './tmp/tmp_mix_size1_confidenceac13.h5'
        ...
        sub_high_x,sub_high_y,sub_high_index,sub_high_y_onehost, sub_inventory_feature,pred_probs_whole= get_high_quality_data_only_confidence_auto_return(res_model, sub_x, sub_y_onhot, layer_name = layer_name, confidence=confidence, auto=auto_para)
        ...
        sampled_x, sampled_y = confidence_policy_active(sub_x, sub_y, incremental_am_feature.shape[0]*size, pred_probs_whole)
