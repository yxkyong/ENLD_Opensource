...
def eval_enld_main_eminist(noise_rate, args): # step=5
################################################ load data
    noisy_rate = noise_rate

    des = 'eminst_noisy{}_ratio2_1'.format(noisy_rate)
    ...
    # k = 1
        ...
        incremental_y_onehot = get_all_one_hot(incremental_y, class_num=n_classes)
        ...
        vote = args.vote
        filter_para = 0.6
        name = './tmp/tmp_mix_emisnt_1true_origin.h5'

        auto_para = True
        print("noise rate and auto_para:", noise_rate, auto_para)
        ################################################################################ naive training
        res_model = load_model(save_path)
        ### lr = 0.001
        ...
        for i in range(iteration):
            ...
            for j in range(step):# validation_data=(incremental_x, incremental_y_onehot)
                res_model.fit_generator(datagen.flow(sampled_x, sampled_y_one_hot,batch_size=batch_size),epochs=2, #validation_data=(incremental_x, incremental_y_onehot),
                                           steps_per_epoch=sampled_x.shape[0] // batch_size)
    ...
    return pre_list, recall_list, time_list, his_all
...
################################################ load data
    noisy_rate = noise_rate
    des = 'tiny_imagenet_noisy{}_ratio2_1'.format(noisy_rate)
    base_dir = args.data_path
    X_train, X_test, Y_train, Y_test = np.load(base_dir + des+'/X_train55.npy'), np.load( base_dir + des+'/X_test55.npy'), \
                                    np.load( base_dir + des+'/Y_train55.npy'), np.load( base_dir + des+'/Y_test55.npy')  
    X_train = X_train.astype('float32') / 255
    ...
    # k = 1
        ...
        vote = args.vote
        filter_para = 0.6
        name = './tmp/tmp_mix_size1_tiny921.h5'
        # if noisy_rate in ['0.1', '0.2']:
        #     auto_para = False
        # if noisy_rate in ['0.3', '0.4']:
        #     auto_para = True
        ...
        for i in range(5): #sub_high_x sub_high_y_onhot
            ...
            history = res_model.fit_generator(datagen.flow(sampled_x, sampled_y_one_hot,batch_size=batch_size),validation_data=(incremental_x, noisy_y_onehot),epochs=1,
                                   steps_per_epoch=sampled_x.shape[0] // batch_size)
            print(history.history['val_accuracy'])
            if history.history['val_accuracy'][0]>best:
                best = history.history['val_accuracy'][0]
                res_model.save(name)
                ############################################################################################-----------------------------------------
                sub_high_x,sub_high_y,sub_high_index,sub_high_y_onehost, sub_inventory_feature= get_high_quality_data_only_confidence_auto(res_model, sub_x, sub_y_onhot, layer_name = layer_name, confidence=confidence, auto=auto_para)
                sub_high_quality_dic, sub_KD_tree_list = enld_function.generate_dic(sub_inventory_feature, sub_high_x, sub_high_y)
        ...
        for i in range(iteration):
            ...
            for j in range(step):# validation_data=(incremental_x, incremental_y_onehot)
                history = res_model.fit_generator(datagen.flow(sampled_x, sampled_y_one_hot,batch_size=batch_size),epochs=1, #validation_data=(incremental_x, incremental_y_onehot),
                                           steps_per_epoch=sampled_x.shape[0] // batch_size)
                # validate.append(history.history['val_accuracy'])
                predict_prob, incremental_feature = enld_function.feature_extraction(res_model, incremental_x, layer_name = layer_name)#res_model.predict(incremental_x)
                predict_label = np.argmax(np.array(predict_prob), axis=1)
...
if args.dataset=='emnist':
    pre_list, recall_list, time_list, his_all, validate_list = eval_enld_main_eminist(noise_rate=args.noise_rate, args=args)
...
if args.dataset=='tiny_imagenet':
    pre_list, recall_list, time_list, his_all, validate_list = eval_enld_main_tiny_imagenet(noise_rate=args.noise_rate, args=args)
    print("{}_result:".format(args.noise_rate))
    print("precision:",pre_list)
    print("recall:",recall_list)
    print("time:",time_list)
