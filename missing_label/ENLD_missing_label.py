...
################################################ load data
    ...
    base_dir = '/home/3005yxk/ENLD_project_8_31/Data/data_202209/cifar100/'
    ...
    print(index)
    layer_name = 'flatten'
    save_path = '/home/3005yxk/ENLD_project_8_31/ENLD_code/model_gen/cifar100/model_results/cifar_100/' + des+'/'+'AUG_INCVR110F_weights{}.h5'.format(index)
    ...
    # k = 1
        ...
        #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
        #### devide incremental_x incremental_y
        # incremental_x_missing incremental_y_missing
        # incremental_x noisy_y incremental_y
        random_seed = 66

        ratio = args.ratio
        all_index = [i for i in range(incremental_x.shape[0])]
        random.seed(random_seed)
        missing_index = random.sample(all_index, int(ratio*len(all_index)))
        ###### define missing
        incremental_x_missing = incremental_x[missing_index].copy()
        incremental_y_missing = incremental_y[missing_index].copy()

        other_index = [i for i in range(incremental_x.shape[0]) if i not in missing_index]
        ###### define 
        incremental_x = incremental_x[other_index].copy()
        noisy_y = noisy_y[other_index].copy()
        incremental_y = incremental_y[other_index].copy()
        #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
        ...
        for i in range(iteration):
            ...
            for j in range(step):# validation_data=(incremental_x, incremental_y_onehot)
                ...
                ################# give pesudo label
                predict_prob_missing, incremental_feature_missing = enld_function.feature_extraction(res_model, incremental_x_missing, layer_name = layer_name)
                predict_label_missing = np.argmax(np.array(predict_prob_missing), axis=1)
                for o in range(len(predict_label_missing)):
                    if o not in pseudo_label_count:
                        pseudo_label_count[o] = np.zeros((n_classes, ))
                    else:
                        pseudo_label_count[o][predict_label_missing[o]] = pseudo_label_count[o][predict_label_missing[o]] + 1
        ...
        # pseudo_label_count
        pseudo_label = []
        for key in pseudo_label_count:
            label = list(pseudo_label_count[key]).index(max(pseudo_label_count[key]))
            # print(label)
            pseudo_label.append(label)
        # print(pseudo_label)
        missing_accuracy_score= accuracy_score(pseudo_label, incremental_y_missing)
        missing_precision = precision_score(pseudo_label, incremental_y_missing, average='micro')
        missing_recall = recall_score(pseudo_label, incremental_y_missing, average='micro')
        missing_f1_score = f1_score(pseudo_label, incremental_y_missing, average='micro')

        missing_list.append([missing_accuracy_score, missing_precision, missing_recall, missing_f1_score])
    return pre_list, recall_list, time_list, his_all, validate_list, missing_list
...
# ###################################################################
pre_list, recall_list, time_list, his_all, validate_list, missing_list = eval_enld_main(noise_rate='0.2',args=args)
print("0.2_result:")
